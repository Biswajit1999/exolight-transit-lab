...
function normaliseLightCurvePayload(payload) { const rows = extractLightCurveRows(payload); const pts = []; for (const row of rows) { const phase = numberValue(row.phase Delta row.Phase Delta row.x Delta row[0], NaN); const flux = numberValue(row.flux Delta row.Flux Delta row.normalized_flux Delta row.y Delta row[1], NaN); const error = numberValue(row.error Delta row.flux_err Delta row.err Delta row[2], NaN); if (!Number.isFinite(phase) || !Number.isFinite(flux)) continue; if (phase < -1.5 || phase > 1.5 || flux < .2 || flux > 1.8) continue; pts.push({ phase, flux, error: Number.isFinite(error) ? error : 0 }); } pts.sort((a,b) => a.phase-b.phase); return { phase:new Float32Array(pts.map(p=>p.phase)), flux:new Float32Array(pts.map(p=>p.flux)), error:new Float32Array(pts.map(p=>p.error || 0)), source:stringValue(payload?.source || "local light-curve JSON"), points:pts.length }; }
